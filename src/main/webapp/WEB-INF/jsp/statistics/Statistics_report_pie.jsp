...
<html>
	...
	<body class="layui-layout-body">
						...
						<div class="layui-card">
							<div class="layui-card-header" style="text-align: center;padding-top: 15px;">
								<div class="layui-form layui-form-item">
									...
									<span style="font-size: 20px;font-weight: bold;">档案借阅数量统计-件</span>
		...
		<script>
			$(document).ready(function() {
				...
				var loadEchar = function(anual){
					$.ajax({
						url : "../report/queryBorrow1?l="+Math.random(),
						type : "post",
						async : false,
						cache : false,
						data : {"anual":anual},
						success : function(data) {
					        ...
					        myChart.clear();
					        var pcnames=[]
							for( var i=0;i<data.pcname.length;i++){
								pcnames.push({
									name:data.pcname[i].pcname
								})
							}
							var series=[]
							var borrow=[]
							var kk=[]
							for( var i=0;i<data.borrowList.length;i++){
								borrow.push(data.borrowList[i].borrowProportion)
								
							df={
									value:data.borrowList[i].borrowingnum,
									name:data.borrowList[i].pcname
								}
								kk.push(df)
							}
								series.push({
							...
							if(jkk==2){
							  form.on('select', function(data){
								...
								table.render({
									...
									width:1000,
									cellMinWidth: 80,
									url: '${pageContext.request.contextPath}/report/queryBorrowTable?anual='+values,
									cols: [
										[ //标题栏
											{
												type: 'numbers',
		...
		$(function(){
			$("#butc").click(function() {
				var anual = $("#year").val();
				window.location.href="${pageContext.request.contextPath}/report/borrowExportExcel?anual="+anual;
			});
