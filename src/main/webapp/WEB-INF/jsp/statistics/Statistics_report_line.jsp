	...
	<body class="layui-layout-body">
				...
				<div class="layui-body" id="LAY_app_body" style="top:180px;">
					<div class="layui-col-md12">
						<div class="layui-card">
							<div class="layui-card-header" style="text-align: center;padding-top: 15px;">
								<div class="layui-form layui-form-item">
									...
									<span style="font-size: 20px;font-weight: bold;">档案整理质量统计</span>
		...
		$(document).ready(function() {
			...
			var loadEchar = function(anual){
				$.ajax({
					url : "../report/queryRejects1?l="+Math.random(),
					type : "post",
					async : false,
					cache : false,
					data : {"anual":anual},
					success : function(data) {
						...
						var names=[]
						var rejectnum=[]
						var rejectProprotion=[]
						 for(var j=0;j<data.borrowList.length;j++){
							 names.push(data.borrowList[j].quanzongname);
							 rejectnum.push(data.borrowList[j].rejectnum);
							 rejectProprotion.push(data.borrowList[j].rejectProprotion);
						 }
						series.push({
							name: '驳回次数',
							type: 'bar',
		...
		var form = layui.form;
			...
			$("#buta").click(function() {
				$('#container').show();
				$('#demo').hide();
				/*  $("[lay-filter='LAY-table-1']").css('display', 'none');  */
				//要执行一次查询图表的方法
				jkk = 1;
						...
						if(jkk==2){
						  form.on('select', function(data){
							...
							table.render({
								...
								width:1000,
								cellMinWidth: 80,
								url: '${pageContext.request.contextPath}/report/queryRejectsTable?anual='+values,
								cols: [
									[ //标题栏
										{
											type: 'numbers',
		...
		$(function(){
			$("#butc").click(function() {
				var anual = $("#year").val();
				window.location.href="${pageContext.request.contextPath}/report/rejectsExportExcel?anual="+anual;
			});
