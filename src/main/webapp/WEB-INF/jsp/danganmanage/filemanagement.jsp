...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/filemanagement.css" />
		<!-- 让IE8/9支持媒体查询，从而兼容栅格 -->
		<!--[if lt IE 9]>
			...
			<div class="layui-layout layui-layout-admin">
				...
				<div class="layui-body" id="LAY_app_body" style="top:180px;">
					<div class="layui-fluid">
						<div id="tba">
						<c:if test="${zm:buttenPremission('da',sessionScope.user.role.authorities) }">
							<a href="${pageContext.request.contextPath}/danganmanage/goboxmanagement" class="btn-a">以盒管理</a>
							</c:if>
							<c:if test="${zm:buttenPremission('db',sessionScope.user.role.authorities) }">
							<a href="${pageContext.request.contextPath}/danganmanage/gofilemanagement" class="btn-a a">以件管理</a>
						</c:if>
						</div>
							...
							<div class="layui-row layui-col-space10 layui-form-item">
							  <form class="layui-form">
								...
								<div class="layui-col-lg2">
									...
									<div class="layui-input-block">
										<select name="archivefileanual" lay-filter="archivefileanual" id="archivefileanual">
								...
								<div class="layui-col-lg-offset1 layui-col-lg3">
									...
									<c:if test="${zm:buttenPremission('dba',sessionScope.user.role.authorities) }">
							...
							<div class="layui-card layui-clear">
								<div class="layui-card-body">
									...
									<div id="" style="margin: 18px 0px;">
										...
										<span id="countFilePages"></span>页
										<c:if test="${zm:buttenPremission('dbd',sessionScope.user.role.authorities) }">
										<button class="layui-btn layui-btn-sm layui-btn-warm" id="change_period">保管期限变更</button>
										</c:if>
										<c:if test="${zm:buttenPremission('dbe',sessionScope.user.role.authorities) }">
										<button class="layui-btn layui-btn-sm" id="classification_period">密级变更</button>
										</c:if>
										<c:if test="${zm:buttenPremission('dbf',sessionScope.user.role.authorities) }">
										<button class="layui-btn layui-btn-sm layui-btn-danger" id="destruction">鉴定销毁</button>
										</c:if>
										<c:if test="${zm:buttenPremission('dbg',sessionScope.user.role.authorities) }">
						...
						<script type="text/html" id="barDemo">
							...
							<a href="${pageContext.request.contextPath}/danganmanage/gofilesubsidiary" class="layui-btn layui-btn-danger layui-btn-xs">管理明细</a>
						</script>
				   ...
				   $("#countFileNum").html("");
					$("#countFilePages").html("");
					  console.log(data); //得到被选中的值
						  ...
						  $.ajax({
							  url:"${pageContext.request.contextPath}/danganmanage/queryPcByquanzongName2"+"?timestamp="+Math.random(),
							  type:"post",
		   ...
		   layui.use('form',function(){
			   var form=layui.form;
			    form.on('select(pcName)',function(data){
			    	  ...
			    	  $.ajax({
						  url:"${pageContext.request.contextPath}/danganmanage/queryBoxAnualByquanzongNameAndPcName2"+"?timestamp="+Math.random(),
						  type:"post",
							 beforeSend :function(xmlHttp){ 
							        xmlHttp.setRequestHeader("If-Modified-Since","0"); 
							        xmlHttp.setRequestHeader("Cache-Control","no-cache");
							 ...
							 success:function(data){
								...
								$("#towfl3").nextAll().remove();
								 for(var i=0;i<data.length;i++){
									 $("#towfl2").after("<option value="+data[i].archivefileanual+">"+data[i].archivefileanual+"</option>"); 
								 }
					  			      //只有执行了这一步，部分表单元素才会自动修饰成功
					  			      form.render();   
							 },
		    ...
		    layui.use('form',function(){
			   var form=layui.form;
			    form.on('select(archivefileanual)',function(data){
			    	  console.log(data)
			    	  var archivefileanual=data.value;
			    	  var quanzongName=$("#quanzongName").val();
			    	  var pcName=$("#pcName").val();
			    	  $.ajax({
						  url:"${pageContext.request.contextPath}/danganmanage/queryRpNameByquanzongNameAndPcNameAndBoxAnual2"+"?timestamp="+Math.random(),
						  type:"post",
							 beforeSend :function(xmlHttp){ 
							        xmlHttp.setRequestHeader("If-Modified-Since","0"); 
							        xmlHttp.setRequestHeader("Cache-Control","no-cache");
							     },
							 data:{"quanzongName":quanzongName,"pcName":pcName,"archivefileanual":archivefileanual},
							 dataType:"json",
							 cache:false,
							 async:false,
							 success:function(data){
		   ...
		   layui.use('form', function() {
				...
				form.on('select(retentionperiodname)', function(data){
					...
					table.reload('description_tabel', {
						  url: "${pageContext.request.contextPath}/danganmanage/queryAllFile"+"?timestamp="+Math.random()
					...
					$.ajax({
						url:"${pageContext.request.contextPath}/danganmanage/queryFileByboxCondition?meng="+<%=Math.random() %>,
						data:{"quanzongName":quanzongName,"pcName":pcName,"archivefileanual":archivefileanual,"retentionperiodname":retentionperiodname},
						type:"post",
						dataType:"json",
						success:function(num){
							console.log(num)
							 $("#countFileNum").html(num.archiveFileNumber);
							$("#countFilePages").html(num.archiveFilePage);
						},
						error:function(){
			...
			layui.use('table', function() {
				...
				table.render({
					...
					url:'${pageContext.request.contextPath}/danganmanage/selectAllFile'+"?timestamp="+Math.random(),
				    ...
				    var fileNum=${fileNum};
					var filePagesNum=${filePagesNum};
				  $("#countFileNum").html(fileNum);
					$("#countFilePages").html(filePagesNum);
						...
						//密级变更弹出层
							$("#classification_period").click(function() { //#btn为按钮id
								...
								'<div class="layui-form-item" style="padding-top: 15px;">'+
							     ...
							     "<input type='hidden' name='id' id='archivefileid2' value='"+obj.data.archivefileid+"'/>"+
									'<label class="layui-form-label">申请人:</label>'+
									'<div class="layui-input-inline">'+
						 ...
						 //鉴定销毁弹出层
							...
							$("#CD_manufacturing").click(function() { //#btn为按钮id
								...
								'<div class="layui-form-item" style="padding-top: 15px;">'+
							     ...
							     "<input type='hidden' name='id' id='archivefileid4' value='"+obj.data.archivefileid+"'/>"+
									'<label class="layui-form-label">申请人:</label>'+
									'<div class="layui-input-inline">'+
			...
			$().ready(function(){
				...
				$("#fileConditionsBtn").click(function(){
					...
					table.reload('description_tabel', {
						  url:'${pageContext.request.contextPath}/danganmanage/queryFileByConditions',
					...
					$.ajax({
						url:"${pageContext.request.contextPath}/danganmanage/queryFileConditionNum?meng="+<%=Math.random() %>,
