...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/boxsubsidiary.css" />
		<!-- 让IE8/9支持媒体查询，从而兼容栅格 -->
		<!--[if lt IE 9]>
					...
					<div class="layui-fluid">
						<div id="tba">
					 		<span style="line-height: 20px;padding-left: 10px;">
					 			...
					 			<a class="font_color">新增数据管理&gt;&gt;</a>
					 			<c:if test="${zm:buttenPremission('da',sessionScope.user.role.authorities) }">
					 			<a href="${pageContext.request.contextPath}/danganmanage/goboxmanagement" class="font_color">以盒管理&gt;&gt;</a><span>管理明细</span>
					 			</c:if>
					 		</span>
					 	</div>
		...
		//时间插件
			...
			layui.use('table', function() {
				...
				table.render({
					...
					url:'${pageContext.request.contextPath}/danganmanage/queryAllManagerdetails'+"?id="+id+"&timestamp="+Math.random(),
				...
				table.on('tool(description_tabel)', function(obj) {
					...
					var boxanual=obj.data.boxanual;
					...
					if(obj.event === 'destruction') {
						...
						console.log(data)
						window.location.href="${pageContext.request.contextPath}/boxSubmitReview/exportDocument?boxYear="+boxanual+"&retentionperiodid="+retentionperiodid;
					}
			...
			$().ready(function(){
				...
				$("#boxConditionsBtn").click(function(){
					...
					table.reload('description_tabel', {
						  url:'${pageContext.request.contextPath}/danganmanage/queryManagedetailsByConditions'+"?timestamp="+Math.random(),
